...
class PAA:
   ...
   def __init__(self):
      ...
      self.questions = ['O seu Pokémon é do tipo Fogo?',
                        'O seu Pokémon é quadrúpede?', 'O seu pokémon pesa mais de 64kg?']
      self.answers = [False]*len(self.questions)
      self.tree = {'111': 'pikachu'}
      self.curr_question = 0
      self.app = Flask(__name__, template_folder='./pages/html',
   ...
   def routes(self):
      ...
      def index():
         ...
         if request.method == 'POST':
            self.answers[self.curr_question] = request.form.get(
                'positive') != None
